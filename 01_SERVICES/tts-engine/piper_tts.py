...
# Configurar logging (sistema de logs del pipeline)
...
class PiperTTS:
    ...
    def __init__(self, config_path: str = "config/tts_config.json"):
        ...
        # Raíz del proyecto = carpeta que contiene config/ y piper/ (p. ej. voice-assistant)
        config_file = Path(config_path).resolve()
        if config_file.exists():
            self._root = config_file.parent.parent if config_file.name == "tts_config.json" else config_file.parent
        else:
            self._root = Path(__file__).resolve().parent.parent  # tts/ -> voice-assistant
