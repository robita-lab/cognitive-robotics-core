...
# En Jetson sin servidor usa el comando principal: ./scripts/udito/start.sh
set -euo pipefail
...
source "$ROOT/scripts/lib/choose-mode.sh"
...
_run_offline() {
  ...
  echo ""
  exec "$ROOT/scripts/udito/start.sh"
}
...
_run_online() {
  ...
  echo ""
  exec "$ROOT/scripts/server/robot-edge.sh"
}
